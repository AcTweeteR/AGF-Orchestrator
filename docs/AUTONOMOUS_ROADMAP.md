...
| E11 end-to-end autonomous pilots | Director | COMPLETED | E11-T1 through E11-T4 disposable pilot evidence complete; no external mutation | Preserve non-authoritative pilot evidence and retain human-controlled boundaries. |
...
Final readiness status is `READY_FOR_ROADMAP_COMPLETE`. All approved roadmap
work is complete, no READY or PLANNED tasks remain, and the repository remains
subject to the active Constitution, mandatory gates, and human-controlled
CRITICAL boundaries.
...
The bounded inventory identifies the repository's approved capability
interfaces without network scanning or credential access: `CodexAdapter` for
the approved Codex CLI path, `OpenHandsSDKAdapter` for the approved OpenHands
path, `OllamaOpenHandsAdapter` for the optional local loopback path, and
`MockAdapter` only for disposable tests. Interface identity is derived from
the adapter contract and local metadata; provider names do not grant policy,
merge, credential, or authority permissions.

Local discovery remains explicitly diagnostic: the installed
`qwen3.5:9b-q4_K_M` profile is inventory evidence only and is not eligible for
primary or fallback implementation. The inventory records unavailable,
invalid, stale, or unknown capability values as ineligible/UNKNOWN rather than
inferring support. No network probe or secret is required for this evidence.

The discovery canaries are `test_detects_preferred_installed_model` and
`test_missing_model_fails_closed` for bounded local inventory,
`test_invalid_inventory_fails_closed` and
`test_remote_endpoint_is_rejected_before_network_access` for fail-closed
endpoint handling, `test_secret_redaction` and
`test_safe_environment_allowlist_excludes_secret_variables` for secret safety,
`test_scheduler_store_is_project_isolated` and
`test_pointer_for_another_project_cannot_cross_project_boundary` for project
isolation, and the full adapter suites for deterministic restart-safe parsing
and failure behavior. No provider is promoted and no capability registry is
created by E9-T1; versioned profiles and empirical selection remain E9-T2 and
E9-T3 scope.
...
E9-T2 is complete with focused tests, 440-test full-suite validation, Ruff,
...
#### E9-T3 deterministic capability selection evidence

The provider-neutral `CapabilitySelector` consumes explicit policy, privacy,
independence, budget, health, and empirical-evidence gates. Missing gate
evidence fails closed. Candidates are ordered deterministically by approved
priority and stable provider/profile identity; stale, cross-project,
unsupported, UNKNOWN, diagnostic-only, or gate-ineligible candidates are
rejected. Fallback is explicit, bounded, and recorded in the selection result;
it cannot override policy or grant provider authority. The local
`qwen3.5:9b-q4_K_M` identity remains diagnostic-only even if caller metadata
claims otherwise.

E9-T3 is complete with 11 focused tests, 440-test full-suite validation,
Ruff, `git diff --check`, independent review APPROVE, and Compliance PASS.
No provider SDK, network probing, credential access, Constitution, policy, or
protected authority boundary was changed.

#### E9-T4 deterministic invalidation evidence

`CapabilityInvalidator` binds evidence to provider-state hashes and health
generations, automatically tombstones stale, changed, conflicting, and
superseded evidence, and rejects cross-project use. Tombstones and active
profile/version history are serialized in one canonical hash-bound state
record and restored on restart. Replayed, stale, regressed, or invalidated
profiles remain ineligible; newer versions supersede older evidence without
resurrecting it. No selector, provider authority, or policy threshold is
changed.
...
uncertainty stops the pilot, preserves the last valid state, and requires
deterministic human resolution before continuation.

#### E10 deterministic execution backlog

E10 is decomposed directly from its approved scope and acceptance criteria.
The decomposition adds no learning authority, provider promotion, policy
mutation, or automatic objective/permission change.
...
#### E11 deterministic execution backlog

E11 is decomposed directly from its approved scope and acceptance criteria.
The decomposition is disposable-only and adds no production, deployment,
provider, authority, policy, objective, permission, or constitutional scope.
...
| E11-T2 (`COMPLETED`) | Prove restart, idempotency, isolation, and failure handling. | Exercise bounded pilot state transitions and evidence recovery without external mutation. | E11-T1 | `src/agf_orchestrator/project_pilot.py`; `tests/test_project_pilot_runs.py`; `docs/AUTONOMOUS_ROADMAP.md` | Repeated operations are deterministic; restart preserves only valid state; failure and cross-project access fail closed. | Failure injection, restart, idempotency, isolation, bounded-state, independent review, and Compliance PASS. | HIGH: state leakage could corrupt pilot conclusions. | Discard failed pilot state; preserve last valid snapshot. |
| E11-T3 (`COMPLETED`) | Prove rollback and human-controlled boundary evidence. | Exercise disposable rollback, kill-switch observation, and owner-boundary checks without changing active policy or production state. | E11-T2, E6-T5, ADR-0003 | `src/agf_orchestrator/project_pilot.py`; `tests/test_project_pilot_boundaries.py`; `docs/AUTONOMOUS_ROADMAP.md` | Rollback is bounded and auditable; kill-switch and CRITICAL boundaries remain authoritative; uncertainty stops the pilot, preserves the last valid state, and requires deterministic human resolution. | Rollback, stale-state, kill-switch, CRITICAL, uncertainty, and authorization-boundary canaries; independent review and Compliance PASS. | CRITICAL: boundary evidence could be mistaken for authority. | Stop pilot and retain authoritative baseline; owner action remains required for CRITICAL operations. |
| E11-T4 (`COMPLETED`) | Run the end-to-end disposable project pilot. | Compose intake, bounded execution simulation, restart/failure/isolation evidence, rollback evidence, and final attributable report. | E11-T1, E11-T2, E11-T3 | `src/agf_orchestrator/project_pilot.py`; `tests/test_end_to_end_project_pilot.py`; `docs/AUTONOMOUS_ROADMAP.md` | No real project or external system changes; every required gate and boundary remains enforced; report is deterministic and non-authoritative. | Full pilot, failure/restart/idempotency/isolation/rollback canaries; full suite, Ruff, diff check, independent review, and Compliance PASS. | HIGH: composition defects could falsely signal readiness. | Disable disposable pilot; retain known-good baseline and non-terminal roadmap state. |

#### E10-T1 bounded outcome-evidence record

`LearningEvidence` records one bounded, attributable outcome with explicit
SUCCESS/FAILURE/BLOCKED status, bounded score delta, source, timestamp, project
and subject identity, and canonical content hashing. `LearningEvidenceLedger`
enforces project isolation, idempotent replay, contradiction rejection, secret
safety, schema/version bounds, and hash-bound restart/readback. It does not
compute scores, alter authority, or apply learning changes.

E10-T1 is complete with 11 focused tests, 462-test full-suite validation,
Ruff, `git diff --check`, independent review APPROVE, and Compliance PASS.

#### E10-T2 bounded confidence and regression summary evidence

`LearningSummary` canonicalizes accepted evidence, binds the summary hash to
the ordered evidence content hashes, bounds score/confidence/prior-score
inputs, and flags deterministic regression without mutating a prior baseline.
Stale, future, duplicate, inconsistent, malformed, or contradictory evidence
fails closed. One result is deliberately damped and cannot create an extreme
permanent score.

E10-T2 is complete with 7 focused tests, 462-test full-suite validation, Ruff,
`git diff --check`, independent review APPROVE, and Compliance PASS.

#### E10-T3 owner-visible learning proposal evidence

`LearningProposal` records attributable, reversible, project-isolated proposal
metadata with canonical content hashing. `LearningProposalLedger` supports
idempotent record, withdraw, and restart/readback only; it exposes no apply or
activation operation. Canonical target validation rejects protected fields and
their variants, and malformed, secret-shaped, conflicting, or cross-project
...
#### E11-T2 bounded transition and recovery evidence

`PilotRunLedger` composes only bounded in-memory transitions for a previously
accepted intake. It enforces exact project/policy binding, monotonic budgeted
steps, terminal failure/completion, operation idempotency, conflicting replay
rejection, restart/readback hashes, and no external mutation. Malformed or
tampered state fails closed.

E11-T2 is complete with 7 focused tests, 486-test full-suite validation, Ruff,
`git diff --check`, independent review APPROVE, and Compliance PASS.

#### E11-T3 rollback and kill-switch boundary evidence

`BoundaryPilot` observes only disposable authority evidence. It verifies pinned
rollback targets, generation advancement, stale authorization invalidation,
project isolation, kill-switch blocking, human-required CRITICAL status, and
incomplete or mismatched evidence rejection. It exposes no activation,
deactivation, rollback, delivery, signature, or authority-mutation API.

E11-T3 is complete with 8 focused tests, 494-test full-suite validation, Ruff,
`git diff --check`, independent review APPROVE, and Compliance PASS.

#### E11-T4 end-to-end disposable project-pilot evidence

`EndToEndPilot` composes accepted intake, bounded successful and failed run
transitions, restart/readback, rollback evidence, kill-switch observation, and
...
- E5 is complete through E5-T3 and PR #61. E6-T1 through E6-T8 are complete
  ...
  is complete through E11-T4. No READY or PLANNED work remains. Final readiness
  is `READY_FOR_ROADMAP_COMPLETE`.
